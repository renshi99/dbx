...
export type QuickConnectionOpenTarget = { kind: "mq-admin" } | { kind: "xxljob" } | { kind: "jenkins" } | { kind: "nacos-admin" } | { kind: "etcd" } | { kind: "zookeeper" } | { kind: "consul" } | { kind: "query"; database: string };
...
export function quickConnectionOpenTarget(connection: Pick<ConnectionConfig, "db_type" | "database">, databaseOptions: string[] = []): QuickConnectionOpenTarget {
  ...
  if (connection.db_type === "xxljob") return { kind: "xxljob" };
  if (connection.db_type === "jenkins") return { kind: "jenkins" };
